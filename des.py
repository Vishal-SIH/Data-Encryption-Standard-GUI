...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        # Title label
        self.instruction = Label(self, text = "Data Encryption Standard", font=("arial",17,"bold"))
        ...
        # Submit 
        self.submit_button = Button(self, text = "Submit", command = self.des)
    ...
    def des(self):
        ...
        def encrypt(msg, key):
            ...
            # permutate by table Permutation_Check_1
            key = permutation_by_table(key, 64, Permutation_Check_1) # 64bit -> Permutation_Check_1 -> 56bit
         
            # split up key in two halves
            # generate the 16 round keys
            C0 = key >> 28
            D0 = key & (2**28-1)
            round_keys = round_keys_gen(C0, D0) # 56bit -> Permutation_Check_2 -> 48bit
            ...
            for i in range(1,17):
                ...
                L_round = R_last
                R_round = L_last ^ round(R_last, round_keys[i])
                L_last = L_round
                R_last = R_round
        ...
        def round_keys_gen(C0, D0):
            ...
            # now form the keys from concatenated CiDi 1<=i<=16 and by apllying Permutation_Check_2
            for i, (Ci, Di) in round_keys.items():
                Ki = (Ci << 28) + Di
                round_keys[i] = permutation_by_table(Ki, 56, Permutation_Check_2) # 56bit -> 48bit
        ...
        cipher_text = '{:x}'.format(cipher_text)
        cipher_text = cipher_text.upper()

        self.result.delete(0.0, END)
        self.result.insert(0.0, cipher_text )
        return cipher_text
